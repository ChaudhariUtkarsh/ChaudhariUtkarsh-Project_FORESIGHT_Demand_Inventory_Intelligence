...
if os.path.exists(model_path):
    ...
    # Fallback: compute correlation-based proxy importance
...
# ── 6. Risk Distribution ──────────────────────────────────────────────────────
# Simulate risk scores using on_hand_units vs rolling mean demand
df_risk = df.copy()
df_risk["forecast_demand"] = (
    df_risk.groupby("sku_id")["units_sold"]
...
axes[1].bar(risk_counts.index, risk_counts.values,
            color=[colors[l] for l in risk_counts.index], edgecolor="white")
