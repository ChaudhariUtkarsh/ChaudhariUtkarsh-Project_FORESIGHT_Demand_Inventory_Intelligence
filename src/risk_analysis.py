...
def perform_risk_analysis(df):
    ...
    if "on_hand_units" in df.columns:
        ...
        logger.warning("Inventory column not found. " "Current inventory will be set to 0.")
        current_inventory = (df.groupby("sku_id").size().astype(float) * 0)

    average_daily_demand = (df.groupby("sku_id")["units_sold"].mean().fillna(0))
    price_column = None
    ...
    if price_column:
        ...
        logger.warning("Price column not found. " "Average Unit Price will be set to 0.")
    ...
    risk_df["Stockout_Risk_Score"] = np.select([risk_df["Days_of_Supply"] <= 3, risk_df["Days_of_Supply"] <= 7, risk_df["Days_of_Supply"] <= 14], [1.00, 0.75, 0.40], default=0.10)
    risk_df["Overstock_Risk_Score"] = np.select([risk_df["Days_of_Supply"] >= 8.0, risk_df["Days_of_Supply"] >= 6.0, risk_df["Days_of_Supply"] >= 4.0], [1.00, 0.75, 0.40], default=0.10)
    ...
    risk_df["Risk_Level"] = np.select([((risk_df["Stockout_Risk_Score"] >= 0.75) | (risk_df["Overstock_Risk_Score"] >= 0.75)), ((risk_df["Stockout_Risk_Score"] >= 0.40) | (risk_df["Overstock_Risk_Score"] >= 0.40))], ["HIGH", "MEDIUM"], default="LOW")
    risk_df["Primary_Risk"] = np.select([risk_df["Stockout_Risk_Score"] > risk_df["Overstock_Risk_Score"], risk_df["Overstock_Risk_Score"] > risk_df["Stockout_Risk_Score"]], ["STOCKOUT", "OVERSTOCK"], default="BALANCED")
    ...
    risk_df = risk_df[["sku_id","Current_Inventory", "Average_Daily_Demand", "Average_Unit_Price", "Days_of_Supply", "Stockout_Risk_Score", "Overstock_Risk_Score", "Risk_Level", "Primary_Risk", "Recommended_Action", "Sales_at_Risk", "Capital_Locked"]]
    return risk_df
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\nTotal SKUs analysed : " f"{len(risk_df)}")
