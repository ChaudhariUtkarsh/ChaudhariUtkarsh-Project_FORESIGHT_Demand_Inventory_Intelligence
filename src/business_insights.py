...
def calculate_business_insights(df):
    ...
    numeric_columns = ["Current_Inventory", "Average_Daily_Demand", "Average_Unit_Price", "Days_of_Supply", "Stockout_Risk_Score", "Overstock_Risk_Score", "Sales_at_Risk", "Capital_Locked"]
    ...
    top_10 = (
        df.sort_values(by=["Risk_Priority", "Stockout_Risk_Score", "Overstock_Risk_Score", "Sales_at_Risk"], ascending=[False, False, False, False]).head(10).copy())

    top_10 = top_10.drop(columns=["Risk_Priority"], errors="ignore")
    return (df, top_10, total_sales_at_risk, total_capital_locked, high_count, medium_count, low_count, stockout_count, overstock_count, balanced_count)


def save_business_insights(top_10, total_sales_at_risk, total_capital_locked, high_count, medium_count, low_count, stockout_count, overstock_count, balanced_count):
...
def print_business_insights(df, top_10, total_sales_at_risk, total_capital_locked, high_count, medium_count, low_count, stockout_count, overstock_count, balanced_count):
    print("\n")
    print("=" * 90)
