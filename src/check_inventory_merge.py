...
def main():
    ...
    if inventory is not None:
        ...
        if inventory_date_col:
            ...
            print(
                ...
                f"{inventory_date_col}"
            )

            print(
                f"Unique inventory dates: "
                f"{inventory[inventory_date_col].nunique():,}"
            )

            print(
                f"First inventory date: "
                f"{inventory[inventory_date_col].min()}"
            )

            print(
                f"Last inventory date: "
                f"{inventory[inventory_date_col].max()}"
            )
    ...
    if weekly is not None:
        ...
        print(
            f"Weekly unique dates: "
            f"{weekly['week_start'].nunique():,}"
        )

        print(
            f"Weekly first date: "
            f"{weekly['week_start'].min()}"
        )

        print(
            f"Weekly last date: "
            f"{weekly['week_start'].max()}"
        )
    ...
    if inventory is not None and weekly is not None:
        if inventory_date_col is not None:
            ...
            print(
                f"Weekly-only dates: "
                f"{len(weekly_only_dates):,}"
            )

            print(
                f"Inventory-only dates: "
                f"{len(inventory_only_dates):,}"
            )
    ...
    if inventory is not None and weekly is not None:
        if ("sku_id" in inventory.columns and inventory_date_col is not None):
            ...
            print(
                f"\nWeekly SKU-date keys : "
                f"{total_keys:,}"
            )

            print(
                f"Matched SKU-date keys: "
                f"{matched_keys:,}"
            )

            print(
                f"Missing SKU-date keys : "
                f"{missing_keys:,}"
            )

            print(
                f"Match percentage      : "
                f"{matched_keys / total_keys * 100:.2f}%"
            )

            print(
                f"Missing percentage    : "
                f"{missing_keys / total_keys * 100:.2f}%"
            )
