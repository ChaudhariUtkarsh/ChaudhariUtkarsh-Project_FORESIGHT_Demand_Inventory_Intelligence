...
WEEKLY_DATA_PATH = os.path.join(DATA_DIR, "weekly_model_data.csv")
...
FEATURES = ["year", "month", "week", "quarter", "lag_1", "lag_2", "lag_4", "rolling_mean_4", "rolling_std_4", "rolling_mean_8", "avg_unit_price", "promotion_rate", "inventory_gap", "total_inventory", "on_hand_units", "on_order_units", "reorder_point", "sku_id_enc"]
...
def prepare_weekly_data():
    ...
    daily = (df.groupby(["sku_id", "date"], as_index=False)
    ...
    daily["week_start"] = (daily["date"] - pd.to_timedelta(daily["date"].dt.dayofweek, unit="D")).dt.normalize()
    weekly = (daily.sort_values(["sku_id", "date"])
        .groupby(["sku_id", "week_start"], as_index=False)
        .agg(weekly_units_sold=("units_sold", "sum"),
    ...
    weekly["rolling_mean_4"] = (g["weekly_units_sold"].transform(lambda s: s.shift(1).rolling(4, min_periods=1).mean()))
    weekly["rolling_std_4"] = (g["weekly_units_sold"].transform(lambda s: s.shift(1).rolling(4, min_periods=2).std()))
    weekly["rolling_mean_8"] = (g["weekly_units_sold"].transform(lambda s: s.shift(1).rolling(8, min_periods=1).mean()))


    for col in ["avg_unit_price", "promotion_rate", "reorder_point", "on_hand_units", "on_order_units"]:
...
class ModelTrainer:
    ...
    def prepare_xy(self):
        ...
        self.df["sku_id_enc"] = (
            self.le.fit_transform(self.df["sku_id"]))
    ...
    def baseline_cv(self, n_splits=N_SPLITS):
        ...
        for (_, train_dates, val_dates) in rolling_origin_splits(self.df["week_start"], n_splits):
            ...
            val = self.df[(self.df["week_start"] >= val_start)& (self.df["week_start"] <= val_end)].copy()
            ...
            val = val.merge(lookup[["sku_id", "source_week", "baseline_forecast"]],
                left_on=["sku_id", "week_start"],
                right_on=[ "sku_id", "source_week"], how="left"
            )

            means = (train.groupby("sku_id")[TARGET].mean())
            val["baseline_forecast"] = (
                val.apply(lambda r: r["baseline_forecast"]
                    if pd.notna(r["baseline_forecast"])
                    else means.get(r["sku_id"], 0),
                    axis=1).clip(lower=0)
            )
    ...
    def train(self):
        ...
        model_configs = {"xgboost": (XGBRegressor, dict(n_estimators=350, learning_rate=0.05, max_depth=6, subsample=0.9, colsample_bytree=0.9, random_state=42, objective="reg:squarederror", verbosity=0)),
            "lightgbm": (LGBMRegressor,dict(n_estimators=350, learning_rate=0.05, max_depth=6, num_leaves=31, random_state=42, verbosity=-1))
        }
        results = {}
        ...
        results["seasonal_naive"] = {"CV_WAPE (%)": round(baseline_wape_value, 2)}
