...
if not os.path.exists(BEST_MODEL_PATH):
    raise FileNotFoundError(f"best_model.pkl not found at: " f"{BEST_MODEL_PATH}. " f"Run src/train_model.py first.")

if not os.path.exists(WEEKLY_DATA_PATH):
    raise FileNotFoundError(f"weekly_model_data.csv not found at: " f"{WEEKLY_DATA_PATH}. " f"Run src/train_model.py first.")
...
FEATURES = metadata.get("features", ["year", "month", "week", "quarter",
                                "lag_1", "lag_2", "lag_4", "rolling_mean_4",
                                "rolling_std_4", "rolling_mean_8", "avg_unit_price", 
                                "promotion_rate", "inventory_gap", "total_inventory", 
                                "on_hand_units", "on_order_units", "reorder_point", "sku_id_enc",
                            ]
                        )
...
def _validate_horizon(forecast_weeks):
    ...
    if not (MIN_WEEKS <= forecast_weeks <= MAX_WEEKS):
        raise ValueError(f"Forecast horizon must be " f"between {MIN_WEEKS} " f"and {MAX_WEEKS} weeks.")
...
def _sku_history(sku_id):
    ...
    if label_encoder is not None:
        ...
        if sku_id not in valid_skus:
            raise ValueError(f"SKU {sku_id} " f"not found in trained model.")
    history = weekly_history[weekly_history["sku_id"] == sku_id].sort_values("week_start").copy()
    if history.empty:
        raise ValueError(f"No weekly history found " f"for SKU {sku_id}.")
    return sku_id, history


class DemandPredictor:
    """Recursive weekly SKU-level predictor for a 6-8 week horizon."""
    def __init__(self):
    ...
    def forecast(self, sku_id, forecast_weeks=6):
        ...
        for step in range(1, forecast_weeks + 1):
            ...
            lag_4 = (demand_history[-4] if len(demand_history) >= 4 else lag_1)
            recent4 = (demand_history[-4:] if demand_history else [0.0])
            recent8 = (demand_history[-8:] if demand_history else [0.0])
            ...
            row = {
                ...
                "week": int(future_week.isocalendar().week),
                "quarter": future_week.quarter,
                "lag_1": lag_1,
                "lag_2": lag_2,
                "lag_4": lag_4,
                "rolling_mean_4": float(np.mean(recent4)),
                "rolling_std_4": float(np.std(recent4)) if len(recent4) > 1 else 0.0,
                "rolling_mean_8": float(np.mean(recent8)),
                "avg_unit_price": avg_unit_price,
                "promotion_rate": promotion_rate,
                "inventory_gap": on_hand - reorder_point,
                "total_inventory": on_hand + on_order,
                "on_hand_units": on_hand,
                "on_order_units": on_order,
                "reorder_point": reorder_point,
                "sku_id_enc": encoded_sku,
            }
            ...
            rows.append({
                ...
                "lower_bound_80": round(lower, 2),
                "upper_bound_80": round(upper, 2),
                "on_hand_units": round(on_hand, 2),
                "on_order_units": round(on_order, 2),
        ...
        return {
            ...
            "total_forecast_units": round(float(forecast_df["predicted_demand"].sum()), 2),
    ...
    def predict(self, data):
        ...
        return {
            ...
            "lower_bound_80": round(sum(x["lower_bound_80"]for x in result["forecast"]), 2),
            "upper_bound_80": round(sum(x["upper_bound_80"]for x in result["forecast"]),  2),
        }