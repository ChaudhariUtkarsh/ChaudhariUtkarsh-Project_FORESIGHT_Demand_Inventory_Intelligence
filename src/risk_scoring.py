...
def enrich_inventory_information(df):
    ...
    if not os.path.exists(PROCESSED_DATA_PATH):
        raise FileNotFoundError(f"processed_data.csv not found:\n" f"{PROCESSED_DATA_PATH}")
    ...
    df.loc[zero_safety, "safety_level"] = (df.loc[zero_safety, "reorder_point"] * SAFETY_MULTIPLIER)
    return df
...
def create_model_features(df):
    ...
    if not os.path.exists(LABEL_ENCODER_PATH):
        raise FileNotFoundError(f"label_encoder.pkl not found:\n" f"{LABEL_ENCODER_PATH}")
    encoder = joblib.load(LABEL_ENCODER_PATH)
    known_skus = set(encoder.classes_.astype(str))

    df["sku_id_enc"] = (df["sku_id"].astype(str).map(lambda x: int(encoder.transform([x])[0])if x in known_skus else -1))
    numeric_features = [
        "year",
        "month",
        "week",
...
def get_model_features():
    if not os.path.exists(METADATA_PATH):
        raise FileNotFoundError(f"model_metadata.json not found:\n" f"{METADATA_PATH}")
...
def calculate_forecast_demand(df):
    ...
    if (isinstance(model_object, dict) and model_object.get("model_type") == "seasonal_naive"):
        ...
        for _, row in df.iterrows():
            ...
            previous_date = (current_date - pd.Timedelta(weeks=season_length))
            key = (sku, previous_date)

            if key in lookup.index:
                forecast = float(lookup.loc[key])
            else:
                forecast = float(row.get("weekly_units_sold", 0))
...
def calculate_risk_score(df):
    ...
    if missing:
        raise ValueError(f"Missing risk columns:\n{missing}")
    ...
    df["lead_time_demand"] = (df["forecast_weekly_demand"] * (df["lead_time_days"] / DAYS_PER_WEEK))
    ...
    df["coverage_pressure"] = np.clip(((8 - df["inventory_coverage_weeks"]) / 8) * 100, 0, 100)
    ...
    df["risk_level"] = np.select([df["risk_score"] >= HIGH_RISK_THRESHOLD, df["risk_score"] >= MEDIUM_RISK_THRESHOLD],["HIGH", "MEDIUM"], default="LOW")
    df["stockout_risk"] = np.where((df["projected_inventory"] <= 0) | (df["inventory_coverage_weeks"] <= 2), "YES", "NO")

    df["risk_reason"] = np.select(
        [
            df["stockout_risk"] == "YES",
            df["inventory_coverage_weeks"] <= 4,
    ...
    df["reorder_priority"] = np.select(
        [
            (df["risk_score"] >= 80) & (df["reorder_quantity"] > 0),
            (df["risk_score"] >= 70) & (df["reorder_quantity"] > 0),
            (df["risk_score"] >= 40) & (df["reorder_quantity"] > 0),
            (df["risk_score"] >= 25) & (df["reorder_quantity"] > 0)
        ],
    ...
    df["inventory_status"] = np.select(
        [df["projected_inventory"] <= 0, df["inventory_coverage_weeks"] <= 4, df["projected_inventory"] < df["safety_level"], df["inventory_coverage_weeks"] >= 12],
    ...
    coverage_excess_pressure = np.clip(((df["inventory_coverage_weeks"] - 8) / 8 ) * 100, 0, 100)
    ...
    df["markdown_clear_priority"] = np.select(
        [
            df["risk_level"] == "HIGH",
            (df["markdown_clear_score"] >= 70) & (df["excess_inventory_units"] > 0),
            (df["markdown_clear_score"] >= 40) & (df["excess_inventory_units"] > 0),
            (df["markdown_clear_score"] >= 20) & (df["excess_inventory_units"] > 0)
    ...
    df["markdown_clear_reason"] = np.select(
        [
            df["risk_level"] == "HIGH",
            df["inventory_coverage_weeks"] >= 16,
            (df["inventory_coverage_weeks"] >= 12) & (df["excess_inventory_units"] > 0),
            (df["excess_inventory_units"] > 0) & (df["forecast_weekly_demand"] < df["available_inventory"])
        ], ["High risk inventory - do not markdown", "Very high inventory coverage", "High coverage with excess inventory", "Inventory exceeds forecast demand"],
        default="No significant markdown requirement"
    )

    df["markdown_discount_percent"] = np.select([df["markdown_clear_priority"] == "HIGH", df["markdown_clear_priority"] == "MEDIUM", df["markdown_clear_priority"] == "LOW"], [30, 20, 10], default=0)
    ...
    df["recommended_action"] = np.select(
        ...
        ["URGENT REORDER", "URGENT REORDER", "MONITOR AND PLAN REORDER", "PLAN REORDER", "PRIORITISE MARKDOWN / CLEARANCE", "CONSIDER PROMOTIONAL PRICING", "CONSIDER MARKDOWN"],
    ...
    df["final_inventory_decision"] = np.select(
        ...
        ["URGENT REORDER", "REORDER", "PLAN REORDER", "CLEAR INVENTORY", "MARKDOWN", "CONSIDER MARKDOWN" ], default="HOLD / MONITOR"
    )
    return df
...
def run_risk_scoring():
    ...
    if not os.path.exists(WEEKLY_DATA_PATH):
        raise FileNotFoundError(f"weekly_model_data.csv not found:\n" f"{WEEKLY_DATA_PATH}")
    if not os.path.exists(BEST_MODEL_PATH):
        raise FileNotFoundError(f"best_model.pkl not found:\n" f"{BEST_MODEL_PATH}")
    ...
    print("\n")
    print("=" * 70)
    print("STOCKOUT SUMMARY")
    print("=" * 70)
    print(result["stockout_risk"].value_counts().to_string())
    ...
    print(f"Minimum: {result['risk_score'].min():.2f}")
    print(f"Maximum: {result['risk_score'].max():.2f}")
