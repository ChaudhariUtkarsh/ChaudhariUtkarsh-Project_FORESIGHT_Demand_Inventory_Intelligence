...
def classify_decision(row):
    stockout_score = float(row["Stockout_Risk_Score"])
    overstock_score = float(row["Overstock_Risk_Score"])~
    primary_risk = str(row["Primary_Risk"]).strip().upper()

    if primary_risk == "OVERSTOCK" and overstock_score >= 0.75:
        return "Markdown / Clear"
    elif primary_risk == "STOCKOUT" and stockout_score >= 0.75:
        return "Reorder Now"
    elif (stockout_score >= 0.40 or overstock_score >= 0.40):
...
primary_distribution = df["Primary_Risk"].value_counts()

for risk in ["STOCKOUT", "OVERSTOCK", "BALANCED"]:
    count = primary_distribution.get(risk, 0)
    print(f"{risk:<25}: {count} SKUs")
...
print("\nTOP WATCH / VOLATILE SKUs")
print("-" * 80)

watch_df = df[
    df["Decision_Quadrant"] == "Watch / Volatile"].sort_values(by="Sales_at_Risk", ascending=False)

if len(watch_df) > 0:
    print(watch_df[["sku_id", "Days_of_Supply", "Stockout_Risk_Score", "Overstock_Risk_Score", "Risk_Level", "Primary_Risk", "Decision_Quadrant", "Decision_Action"]].head(10).to_string(index=False))
else:
    print("No SKUs classified as Watch / Volatile.")


print("\nTOP HEALTHY SKUs")
print("-" * 80)

healthy_df = df[df["Decision_Quadrant"] == "Healthy"]

if len(healthy_df) > 0:
    print(healthy_df[["sku_id", "Days_of_Supply", "Stockout_Risk_Score", "Overstock_Risk_Score", "Risk_Level", "Primary_Risk", "Decision_Quadrant", "Decision_Action"]].head(10).to_string(index=False))
else:
    print("No SKUs classified as Healthy.")
...
markdown_capital = markdown_df["Capital_Locked"].sum()
reorder_sales_at_risk = reorder_df["Sales_at_Risk"].sum()

print(f"Total Sales at Risk      : {total_sales_at_risk:,.2f}")
print(f"Total Capital Locked     : {total_capital_locked:,.2f}")
print(f"Markdown Capital         : {markdown_capital:,.2f}")
print(f"Reorder Sales at Risk    : {reorder_sales_at_risk:,.2f}")
