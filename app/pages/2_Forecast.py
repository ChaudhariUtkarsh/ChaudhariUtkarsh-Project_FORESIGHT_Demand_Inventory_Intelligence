...
seasonal_history = seasonal_history[["week_start", "seasonal_naive"]].copy()
seasonal_history = seasonal_history.rename(columns={"week_start": "date"})

ml_forecast = forecast_df[["week_start", "predicted_demand"]].copy()
ml_forecast = ml_forecast.rename(columns={"week_start": "date", "predicted_demand": "ml_forecast"})
ml_forecast = ml_forecast.sort_values("date")

comparison_df = actual_history.merge(seasonal_history, on="date", how="left")
comparison_df = comparison_df.sort_values("date")

fig = go.Figure()

fig.add_trace(go.Scatter(x=comparison_df["date"], y=comparison_df["actual_demand"], mode="lines+markers", name="Actual Demand", line=dict(width=2), marker=dict(size=5)))
fig.add_trace(go.Scatter(x=comparison_df["date"], y=comparison_df["seasonal_naive"], mode="lines", name="52-Week Seasonal Naive", line=dict(width=2, dash="dash")))
fig.add_trace(go.Scatter(x=ml_forecast["date"], y=ml_forecast["ml_forecast"], mode="lines+markers", name="ML Forecast", line=dict(width=3), marker=dict(size=7)))

future_seasonal = forecast_df[["week_start", "seasonal_naive_demand"]].copy()
future_seasonal = future_seasonal.rename(columns={"week_start": "date", "seasonal_naive_demand": "seasonal_naive_forecast"})
fig.add_trace(go.Scatter(x=future_seasonal["date"], y=future_seasonal["seasonal_naive_forecast"], mode="lines+markers", name="Seasonal Naive Forecast", line=dict(width=2, dash="dot"), marker=dict(size=6)))

forecast_start = ml_forecast["date"].min()
fig.add_vline(x=forecast_start.timestamp() * 1000, line_dash="dash", annotation_text="Forecast Start", annotation_position="top")

fig.update_layout(title=(f"{sku_id} — Actual Demand vs Forecast " f"({forecast_weeks}-Week Horizon)"), 
                    xaxis_title="Week", yaxis_title="Demand Units", hovermode="x unified", 
                    legend_title="Series", template="plotly_white", height=550
                )
